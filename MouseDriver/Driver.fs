namespace MouseDriver
   ...
   // MOUSE features that have to be notified from the sensor //
    ...
    type MouseSensor(debug) =
        inherit Event<MouseEventArgs>() 
      
        let mutable down = false

        //da vederer bene che cazzo mettere
        let mutable sensorEvent = new Event<_>()
    ...
    *)
        ...
        member this.MDown(e : MouseEventArgs) =
                if debug then printfn "MOUSE DOWN"
            //    let evargs = new MouseEventArgsGestIT(e.Button,e.Clicks,e.X,e.Y,e.Delta)
                sensorEvent.Trigger(e) // (evargs)
                down <- true
                //this.MyTrigger MouseFeatureTypes.MouseDown evargs
              
        member this.MUp(e : MouseEventArgs) =
        ...
        member this.MMove(e : MouseEventArgs) =
